...
namespace Eirene.BLL.Services.Implementation.Core
{
    public class SupervisionService : ISupervisionService
    {
        ...
        public async Task<Result<List<SupervisionRequest>>> GetSupervisionRequestsAsync(string doctorUserId)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                return Result.Failure<List<SupervisionRequest>>("An error occurred while fetching supervision requests.");
            }
        }
